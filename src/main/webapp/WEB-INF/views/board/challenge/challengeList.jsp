<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="semi.edit.greener.member.model.vo.*, semi.edit.greener.board.challenge.model.vo.*, 
    java.util.ArrayList, semi.edit.greener.board.model.vo.*, semi.edit.greener.notice.model.vo.*"%>
<%
	ArrayList<Notice> fixedList = (ArrayList<Notice>)request.getAttribute("fixedList");
   ArrayList<Challenge> chList = (ArrayList<Challenge>)request.getAttribute("list");
  ...
  <div class="row">
   ...
   <div class="container">
	...
	<!-- 				</div> -->
      ...
      <% if(m != null) { %>
      <div class="mt-4 mb-3 text-lg-end" style="font-size: 18px;" onclick="location.href='<%= request.getContextPath()%>/challengeWriteForm.ch'">
                        ...
                        <td>
                         <% for(int j = 0; j < imageList.size(); j++){ 
                             ...
                             if(cb.getBoardNo() == img.getBoardNo()){ %>
                                <%-- <img class="img-fluid img-thumbnail" src="${contextPath} }/resources/challenge_image_files/<%= img.getChangeName() %>" alt="챌린지 이미지" width="200" height="200"> --%>
                                <img class="img-fluid img-thumbnail" src="resources/challenge_images/<%= img.getChangeName() %>" alt="챌린지 이미지" width="200" height="200">
                             <% } %>
                           <% } %>
                     </td>
                   </tr>
      ...
      <nav aria-label="Page navigation example">
        <ul class="pagination justify-content-center">
          <li class="page-item">
            <a class="page-link" href="<%= request.getContextPath() %>/list.ch?page=1" aria-label="Previous">
          ...
          <% for(int p = pi.getStartPage(); p <= pi.getEndPage(); p++){ %>
             ...
             <% } else { %>
                <li class="page-item"><button class="page-link" id="numBtn" onclick="location.href='<%= request.getContextPath() %>/list.ch?page=<%= p %>'"><%= p %></button></li>
             <% } %>
          <%} %>
<%--                 <li class="page-item"><button class="page-link" onclick="location.href='<%= request.getContextPath() %>/reportlist.bo?currentPage=<%= p.getCurrentPage() +1 %>'" id="afterBtn">&gt;</button></li> --%>
          <li class="page-item">
            <a class="page-link" href="<%= request.getContextPath() %>/list.ch?page=<%= pi.getMaxPage() %>" aria-label="Next">
