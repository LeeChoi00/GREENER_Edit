<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="semi.edit.greener.board.challenge.model.vo.*, semi.edit.greener.board.model.vo.*"%>
...
<div class="container">
  <div class="row">
    ...
    <div class="col-8">
          <div class="mb-2">
          <br>
    		<form action="<%= request.getContextPath() %>/update.ch" method="post" encType="multipart/form-data">
           <h3 class="fw-bold pt-3 pt-xl-5 pb-2 pb-xl-3"><img src="icon/pencil-square.svg" width="30px" height="30px"> 글쓰기 </h3>
           <input type="hidden" name="boardNo" value="<%= ch.getBoardNo() %>">
...
<!-- 			* 챌린지  -->
			<div class="input-group mb-2" style="width: 50%;">
			  ...
			  <select class="form-select" id="inputGroupSelect01" name="chalName">
			    <option selected>* 참여한 챌린지를 선택해 주세요</option>
			    <option value="1" <%= ch.getChalName().equals("용기내 챌린지")? "selected":"" %> >>용기내 챌린지</option>
			    <option value="2" <%= ch.getChalName().equals("플로깅 챌린지")? "selected":"" %> >>플로깅 챌린지</option>
			    <option value="3" <%= ch.getChalName().equals("제로웨이스트 챌린지")? "selected":"" %> >>제로웨이스트 챌린지</option>
			    <option value="4" <%= ch.getChalName().equals("캔크러시 챌린지")? "selected":"" %> >>캔크러시 챌린지</option>
			  </select>
			</div>
	        <div class="mb-1">
	            <input class="form-control form-control" name="boardTitle" maxlength='20' type="text" value="${ch.boardTitle }" required style="width: 70%;">
	        </div>          	
	        <div class="mb-1">
	            <input class="form-control form-control-md" name="chalPlace" type="text" value="${ch.chalPlace }" style="width: 70%;">
	        </div>          	
			<div class="mb-4">
	        ...
	        <div class="mb-4">
			    ...
			    <textarea class="form-control" name="boardContent" style="height: 700px; resize: none;" required>${ch.boardContent }</textarea>
	        </div>
	        <div class="mb-4 text-end"> 
	        	<input type="button" class="btn btn-outline-secondary" id="cancelBtn" onclick="location.href='<%= request.getContextPath() %>/list.ch'" value="취소">
